...
VOID LightMaterial()
{
    ...
    // Устанавливаем константы в шейдер
    g_pConstantTable->SetFloatArray(pDirect3DDevice, "ambient_color", (float*)&Material.Ambient, 4);
    g_pConstantTable->SetValue(pDirect3DDevice, "ambient_intensity", &intensity, sizeof(float));

    //D3DXVECTOR4 VectorDir;
    ////  Установим свет
    ...
    //Light.Range = 1000.0f;
    ////  Установим нормаль
    //VectorDir = D3DXVECTOR4(-0.65, -0.65, -0.65, 0.0f);
}
...
VOID Matrix()
{
    ...
    // MatrixView
    D3DXMatrixLookAtLH( &MatrixView,  &D3DXVECTOR3 ( 0.0f, 0.0f,-3.0f ),
    ...
    g_pConstantTable->SetMatrix(pDirect3DDevice, "mView", &MatrixView);
...
if (!(hwnd = CreateWindowEx(NULL,              // стиль окна
                       "WINDOWSCLASS",         // класс
					   "Равномерное освещение", // название окна
