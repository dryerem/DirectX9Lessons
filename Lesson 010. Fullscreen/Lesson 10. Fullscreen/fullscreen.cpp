...
LRESULT CALLBACK MainWinProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) // Обработка сообщений
{
	...
	case WM_KEYDOWN: // Завершаем приложение по нажатию Esc
		if (wParam == VK_ESCAPE)
		{
			PostQuitMessage(0);
			return 0;
...
HRESULT InitialDirect3D(HWND hWnd)
{
	...
	// Устанавливаем ширину и высоту заднего буфера размерами экрана
	...
	Direct3DParametr.BackBufferCount = 3; // Устанавливает количество задних буферов (максимум), ускоряет и улучшает качество рендера
	Direct3DParametr.FullScreen_RefreshRateInHz = Display.RefreshRate; // Задаём частоту обновления
	Direct3DParametr.Windowed = FALSE; // Запрещаем оконный режим
